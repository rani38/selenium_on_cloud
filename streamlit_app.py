...
def check_chromedriver_permissions(path):
    if os.path.exists(path):
        st.write(f"'chromedriver.exe' found at: {path}")
        st.write(f"Executable: {os.access(path, os.X_OK)}")
        return os.access(path, os.X_OK)
    else:
        st.write(f"'chromedriver.exe' not found at: {path}")
        return False
...
def download_pdf_using_selenium(url, dir):
    ...
    try:
        if not os.path.exists(path_to_chrome_driver):
            st.error("ChromeDriver executable not found. Please ensure 'chromedriver.exe' is in the correct location.")
            raise FileNotFoundError("ChromeDriver executable not found.")

        if not check_chromedriver_permissions(path_to_chrome_driver):
            st.error("ChromeDriver executable does not have execute permissions.")
            raise PermissionError("ChromeDriver executable does not have execute permissions.")
        ...
        st.error(
            "Error: ChromeDriver executable not found. Please ensure 'chromedriver.exe' is in the correct location.")
...
if st.button("download files"):
    ...
    links = ['https://link.springer.com/content/pdf/10.1007/s00167-018-4953-z.pdf',
